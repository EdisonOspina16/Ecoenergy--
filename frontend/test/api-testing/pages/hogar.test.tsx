...
describe('Página: Dashboard Hogar', () => {
  ...
  beforeEach(() => {
    vi.spyOn(useDispositivosHooks, 'cargarDispositivos').mockImplementation(({ setDevices, setLoadingDevices }) => {
      setDevices([{ nombre: 'Nevera Inteligente', consumo: 30, estado: 'Encendido' }]);
    ...
    global.fetch = vi.fn((url: string | URL | Request) => {
      ...
      if (urlStr.includes('perfil')) {
         return Promise.resolve({
           ok: true,
           json: async () => ({ success: true, hogar: { nombre_hogar: "Mi Hogar Mock", direccion: "123 Mock" } })
         } as Response);
      }
      ...
      if (urlStr.includes('recomendacion-diaria')) {
         return Promise.resolve({
           ok: true,
           json: async () => ({ success: true, recomendaciones: [{ recomendacion: "Apaga la luz", esAlerta: false }] })
         } as Response);
      }
  ...
  it('debería llamar a los servicios correspondientes on mount (Verify Fetch Spies)', async () => {
    ...
    await waitFor(() => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('consumo-historico'), expect.any(Object));
